<template>
  ...
  <div v-else>
    ...
    <FilterBar :class="['fixed z-40', ]" v-model="searchQuery" :dropdowns="searchOptions" horizontal />
...
import { BASE_URL, BOARDS, carTypeOptions, newOriginOptions, statusOptions, titleOrPayoffOptions, usedOriginOptions, CurrentBoardColumns, PreviousBoardColumns, PrintingBoardColumns } from '@mim-workspace/constants'
...
onMounted(async () => {
  try {
    ...
    const allItems = [ ...currentBoardItems, ...previousBoardItems, ...printingBoardItems ]
    ...
    error.value = requestError instanceof Error ? requestError.message : 'Unable to load inventory'
  } finally {
    loading.value = false
  }
})
