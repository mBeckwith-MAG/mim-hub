    ...
    <Container class="relative top-10">
      <div v-if="filteredItems && filteredItems.length">
        <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-x-4">
          <ItemCard v-for="item in filteredItems" :item :key="item.uid" class="mb-8" />
        </div>
      </div>
...
import { BASE_URL, BOARDS, Stores, carTypeOptions, newOriginOptions, statusOptions, titleOrPayoffOptions, usedOriginOptions } from '@mim-workspace/constants'
