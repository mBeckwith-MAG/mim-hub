  ...
  <div>
    <Container class="fixed top-6 left-1/3 flex gap-2x text-2xl font-thin z-60">
      <button 
        ...
        :class="['flex items-center gap-sm cursor-pointer transition-color', { 'text-sky-500 border-b-2 border-sky-500': activeTab === tab.id }]" 
      >
        <Icon :icon="tab.icon" />
        <span>{{ tab.name }}</span>
      </button>
      ...
      <div v-for="tab in processedTabs" :key="tab.id">
        <div v-if="activeTab === tab.id">
          <div v-if="tab.items && tab.items.length">
            <Grid :cols="3">
              <template v-for="(item, index) in tab.items" :key="`${item.uid}-item-${index}`">
